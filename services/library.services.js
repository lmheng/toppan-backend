...
/*const getTop3ReadBook = async (countryCode) => {
  ...
  if (!!numericalCode || numericalCode === 0) {
    ...
    await db.sequelize
      .query(
        `select br.book_id, b.name as book_name, a.name as author_name, COUNT(*) as rented
          from book_rents br, books b, author_books ab, authors a 
          where br.book_id = b.id and ab.book_id = b.id and a.id = ab.author_id
          GROUP BY br.book_id, book_name, author_name order by rented desc limit 3`
      )
      .then(([top3BooksResult, top3BooksMetadata]) =>
        top3BooksResult.forEach((book) => {
          top3BooksWithReaders.push({
    ...
    for (const element of top3BooksWithReaders) {
      element["borrower"] = await getTop3ReadersPerCountry(
        numericalCode,
        element["id"]
      );
    ...
    console.log(top3BooksWithReaders);
    if (top3BooksWithReaders.some((e) => e["borrower"].length === 0)) {
...
const getTop3ReadersPerCountry = async (numericalCode, bookId) => {
  return await db.sequelize
    .query(
      `select br.book_id, p.name, tr.totalSum, rent.rentSum
        from people p
        join book_rents br
          on p.id = br.person_id
        join (
          select p.id, p.name, COUNT(*) as rentSum
          from people p, book_rents br
          where p.id = br.person_id and p.country_id = ${numericalCode} and br.book_id = ${bookId}
          GROUP BY p.id, p.name order by rentSum desc limit 3
        ) rent
          on p.id = rent.id
        join (
          select p.id, COUNT(*) as totalSum from people p, book_rents br
          ...
          on p.id = tr.id
        where p.country_id = ${numericalCode} and br.book_id = ${bookId}
        GROUP BY br.book_id, p.name, tr.totalSum, rentSum order by tr.totalSum desc limit 3`
    )
    .then(([top3ReadersResult, top3ReadersMetadata]) =>
      top3ReadersResult.map((reader) => reader["name"])
    );
};
*/
